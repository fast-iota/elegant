...
char *material, *formula, *str_value, **columnMatch, **parMatch, *inputfile, *outputfile, *energyCol, *energyPar, *phiCol, *phiPar, *thetaCol, *thetaPar;
 ...
 for (i_arg = 1; i_arg < argc; i_arg++) {
   if (s_arg[i_arg].arg_type == OPTION) {
     ...
     case SET_ENERGY:
       ...
       if (energyCol)
         energySpecSource = 10 * energySpecSource + BYCOLUMN;
       if (energyPar)
         energySpecSource = 10 * energySpecSource + BYPARAMETER;
       if (energyValue >= 10)
         energySpecSource = 10 * energySpecSource + BYVALUE;
       if (eStart > 10 && eEnd > 10 && energyPoints > 0)
         energySpecSource = 10 * energySpecSource + BYRANGE;
     ...
     case SET_THETA:
       ...
       if (thetaCol)
         thetaSpecSource = 10 * thetaSpecSource + BYCOLUMN;
       if (thetaPar)
         thetaSpecSource = 10 * thetaSpecSource + BYPARAMETER;
       if (thetaValue < INVALID_ANGLE)
         thetaSpecSource = 10 * thetaSpecSource + BYVALUE;
       if (thetaStart < INVALID_ANGLE && thetaEnd < INVALID_ANGLE && thetaPoints > 0)
         thetaSpecSource = 10 * thetaSpecSource + BYRANGE;
     ...
     case SET_PHI:
       ...
       if (phiCol)
         phiSpecSource = 10 * phiSpecSource + BYCOLUMN;
       if (phiPar)
         phiSpecSource = 10 * phiSpecSource + BYPARAMETER;
       if (phiValue < INVALID_ANGLE)
         phiSpecSource = 10 * phiSpecSource + BYVALUE;
       if (phiStart < INVALID_ANGLE && phiEnd < INVALID_ANGLE && phiPoints > 0)
         phiSpecSource = 10 * phiSpecSource + BYRANGE;
       if (phiSpecSource >= 10)
     ...
     case SET_TARGET:
       ...
       if (material != NULL) {
         if (verbose > 3)
           fprintf(stdout, " User provides target material name. \n");
         ...
         if (verbose > 3)
           fprintf(stdout, " User provides target formula. \n");
         strcpy(targetFormula, formula);
     ...
     case SET_FILTER:
       ...
       if (material != NULL) {
         if (verbose > 3)
           fprintf(stdout, " User provides filter material name. \n");
         ...
         if (verbose > 3)
           fprintf(stdout, " User provides filter formula. \n");
         strcpy(filterFormula, formula);
         if (get_material_property(formula, "name", &str_value, NULL, NULL) < 0)
     ...
     case SET_DETECTORFILTER:
       ...
       if (material != NULL) {
         ...
         if (verbose > 3)
           fprintf(stdout, " User provides detector filter material name. \n");
         if (formula != NULL) {
         ...
         strcpy(detectFilterFormula, formula);
         if (verbose > 3)
           fprintf(stdout, " User provides detector filter formula. \n");
         if (get_material_property(formula, "name", &str_value, NULL, NULL) < 0)
 ...
 calc_mode = mode % 10;
 if (!inputfile) {
   if (energySpecSource != BYVALUE && energySpecSource != BYRANGE)
     SDDS_Bomb("error: energy specifications are invalid or nonexisting!");
   ...
   if (energySpecSource <= 0)
     SDDS_Bomb("error: energy specifications are invalid or nonexisting!");
   if (calc_mode == 0 || calc_mode == 2) {
     if (thetaSpecSource <= 0)
       SDDS_Bomb("error: theta specifications are invalid or nonexisting!");
     if (phiSpecSource <= 0 && fabs(polarization) <= 0.001)
       SDDS_Bomb("error: phi specifications are invalid or nonexisting!");
   }
 }
 ...
 if (!inputfile) {
   ...
   if (thetaSpecSource == BYVALUE) {
     ...
     deltaTheta = 0.0;
     if (npts > 1)
       deltaTheta = (thetaEnd - thetaStart) / (npts - 1);
     for (i = 0; i < npts; i++)
   ...
   if (phiSpecSource == BYVALUE) {
     ...
     deltaPhi = 0.0;
     if (npts > 1)
       deltaPhi = (phiEnd - phiStart) / (npts - 1);
     for (i = 0; i < npts; i++)
...
int getDetectorSignal(long mode) {
  ...
  if (verbose > 3) {
    fprintf(stdout, "getDetectorSignal: cosTargetThetaIn = %f, cosTargetThetaOut = %f\n", cosTargetThetaIn, cosTargetThetaOut);
  }
  ...
  if (verbose > 3) {
    fprintf(stdout, "getDetectorSignal: Z = %d, massThickness = %f\n", Z, massThickness);
  }
  if (verbose > 2) {
...
int getScatterCS(long mode) {
  ...
  if (verbose > 3) {
    fprintf(stdout, "getScatterCS: cosTargetThetaIn = %f, cosTargetThetaOut = %f\n", cosTargetThetaIn, cosTargetThetaOut);
  }
  /* Print verbose mode information */
  if (verbose > 3) {
    fprintf(stdout, "getScatterCS: Z = %d, massThickness = %f\n", Z, massThickness);
  }
  if (verbose > 2) {
    fprintf(stdout, "\n Index PhotonEnergy thetaOut  phiOut comptonEnergy TotalCS  rayleighCS  rayleighDCS   comptonCS   comptonDCS ");
    if (calc_mode == 1) {
      fprintf(stdout, " RayleighYield  ComptonYield");
    }
    if (calc_mode == 2) {
      fprintf(stdout, "RayleighFluxDensity ComptonFluxDensity");
    }
    fprintf(stdout, "\n             (eV)     (deg)   (deg)       (eV)    (cm^2/g)   (cm^2/g) (cm^2/g/sterad)  (cm^2/g) (cm^2/g/sterad)");
    if (calc_mode == 2) {
      fprintf(stdout, "   1/sterad           1/sterad");
    }
  }
  ...
  for (i = 0; i < npts; i++) {
    ...
    if (verbose > 2) {
      ...
      if (calc_mode == 1) {
        fprintf(stdout, "%14.4g %14.4g", totalRayleighYield[i], totalComptonYield[i]);
      }
      if (calc_mode == 2) {
        fprintf(stdout, "%16.4g %18.4g", rayleighFluxDensity[i], comptonFluxDensity[i]);
      }
    }
  }
...
char *modeString(long mode) {
  ...
  msg = "unknown mode.";
  if (mode == 0) {
    msg = "Cross sections of a target using its formula";
  } else if (mode == 1) {
    msg = "Compton yield in a compound target, no self absorption";
  } else if (mode == 2) {
    msg = "Compton flux density from a compound target";
  } else if (mode == 3) {
    msg = "Compton detector signal from a compound target";
  } else if (mode == 4) {
    msg = "Compton detector signal from a two-layer compound target: filter-target";
  } else if (mode == 5) {
    msg = "Compton detector signal from a two-layer compound target with pinhole aperture";
  } else if (mode == 10) {
    msg = "Compton cross sections of an element using its symbol";
  } else if (mode == 11) {
    msg = "Total Comptonyield in an elemental target, no self absorption";
  } else if (mode == 12) {
    msg = "Compton flux density from an elemental target";
  } else if (mode == 13) {
    msg = "Compton detector signal from an elemental target";
  } else if (mode == 14) {
    msg = "Compton detector signal from a two-layer elemental target: filter-target";
  } else if (mode == 15) {
    msg = "Compton detector signal from a two-layer elemental target with pinhole aperture";
  } else if (mode == 20) {
    msg = "Klein-Nishina cross sections of an elemental target using its nominal electron density";
  } else if (mode == 21) {
    msg = "Klein-Nishina Compton yield in an elemental target, no self absorption";
  } else if (mode == 22) {
    msg = "Klein-Nishina Compton flux density from an elemental target";
  }
