...
double FELScalingFunction(double *etaDiffraction, double *etaEmittance, double *etaEnergySpread,
                          double L1D, double beta, double emittance, double lightWavelength, double undulatorPeriod,
                          double sigmaDelta);
void ComputeSASEFELParameters(double *lightWavelength, double *saturationLength, double *gainLength,
...
int OptimizeSASEFELParameters(VALUE_NAME charge, VALUE_NAME rmsBunchLength, VALUE_NAME undulatorPeriod,
...
void ComputeSASEFELParameters(double *lightWavelength, double *saturationLength, double *gainLength, double *noisePower,
      ...
      if ((rows = SDDS_RowCount(&SDDSin)) > 0) {
        ...
        while (ValueName[++i]) {
          if (ValueName[i]->inFile) {
            if (!(ValueName[i]->value = SDDS_GetColumnInDoubles(&SDDSin, ValueName[i]->name)))
              SDDS_PrintErrors(stderr, SDDS_VERBOSE_PrintErrors | SDDS_EXIT_PrintErrors);
          } else
        ...
        for (row = 0; row < rows; row++) {
          if (OptimizeSASEFELParameters(charge, rmsBunchLength, undulatorPeriod, undulatorK,
            ...
            lightWavelength = saturationLength = gainLength = noisePower = saturationPower = 
              PierceParameter = etaDiffraction = etaEmittance = etaEnergySpread = 0;
          }
...
int OptimizeSASEFELParameters(VALUE_NAME charge, VALUE_NAME rmsBunchLength, VALUE_NAME undulatorPeriod,
  ...
  double lower[8] = {1e-15, 1e-15, 1e-3, 0.001, 0.001, 1e-14, 1e-9, 100};
  double upper[8] = {1e-9, 1e-9, 0.1, 10.0, 1000.0, 1e-6, 1e-2, 100e3};
  ...
  for (i = 0; i < 8; i++)
    dx[i] = 0.1 * x0[i];
  if (simplexMin(&result, x0, dx, lower, upper, disable, 8, 0.0, 1e-9,
                 SASEFELOptimFn, NULL, 500, 10, 12, 3.0, 1.0, 0) < 0) {
    success = 0;
