...
int main(int argc, char *argv[]) {
  long i_arg, i, validMode, pages, j, parMatches = 0, colMatches = 0, exist = 0;
  //long Z;
  int value;
  ...
  for (j = 0; j < pages; j++) {
    ...
    if (mode == 20) {
      ...
      for (i = 0; i < nShells; i++) {
        if (verbose) {
          fprintf(stdout, "shellIndex = %ld, shellID = %d\n", i, shellID[i]);
        }
...
int getShellID(char *name) {
  ...
  if (strcmp(name, "K") == 0) {
    ...
    value = N4_SHELL;
  } else if (strcmp(name, "N5") == 0) {
    value = N5_SHELL;
  } else if (strcmp(name, "N6") == 0) {
    value = N6_SHELL;
  } else if (strcmp(name, "N7") == 0) {
    value = N7_SHELL;
    ...
    value = O4_SHELL;
  } else if (strcmp(name, "O5") == 0) {
    value = O5_SHELL;
  } else if (strcmp(name, "O6") == 0) {
    value = O6_SHELL;
  } else if (strcmp(name, "O7") == 0) {
    value = O7_SHELL;
...
int getRefraction(int mode) {
  ...
  if (verbose > 3) {
    fprintf(stdout, "getRefraction: targetFormula = %s\n", targetFormula);
  }
  if (verbose > 2) {
    if (mode == 1) {
  ...
  for (i = 0; i < npts; i++) {
    ...
    if (mode < 10) { /* Compound target */
      ...
      if (mode == 6) { /* Mirror reflectivity */
        ...
        if (verbose > 2) {
          fprintf(stdout, "%12.4g", reflectivity[i]);
        }
      }
...
int getXRayCS(int mode, double massThickness) {
  ...
  if (verbose > 3) {
    fprintf(stdout, "getXRayCS: targetFormula = %s\n", targetFormula);
  }
  if (mode > 9) {
    Z = SymbolToAtomicNumber(targetFormula);
  }
  if (verbose > 2) {
    ...
    if (mode == 2 || mode == 12) {
      fprintf(stdout, "    Transmission  Absorption ");
    } else if (mode == 4 || mode == 14) {
      fprintf(stdout, "  Transmission  Absorption  FrontTEY(QE)  BackTEY(QE)  TEY(QE) ");
    }
  ...
  for (i = 0; i < npts; i++) {
    ...
    if (mode == 2 || mode == 4 || mode == 12 || mode == 14) {
      ...
      if (verbose > 2) {
        fprintf(stdout, " %12.4f %12.4g", transmission[i], absorption[i]);
      }
      if (mode == 4 || mode == 14) {
        ...
        if (verbose > 2) {
          fprintf(stdout, "%12.4g %12.4g %12.4g", eYieldFront[i], eYieldBack[i], electronYield[i]);
        }
      }
    }
  }
...
int getAtomicXRayCS_Kissel(int shellID) {
  ...
  if (verbose > 3) {
    ...
    if (verbose > 2) {
      fprintf(stdout, "Index  PhotonEnergy   TotalCS      PhotoCS     coherentCS   incohrentCS \n");
    }
  }
  ...
  if (shellID <= 30 && shellID >= 0) {
    ...
    for (i = 0; i < npts; i++) {
      ...
      compton[i] = 0.0;
      if (verbose > 2) {
        fprintf(stdout, "%4d %12.1f %12.4g %12.4g %12.4g %12.4g \n", i, energy[i], total[i], photo[i], rayleigh[i], compton[i]);
      }
    }
  } else if (shellID > 99) {
    for (i = 0; i < npts; i++) {
