      ...
      if (includeDispersion &&
          (!SDDS_SetParameters(&SDDSout, SDDS_SET_BY_NAME | SDDS_PASS_BY_VALUE,
           ...
           (error_output &&
            !SDDS_SetParameters(&SDDSout, SDDS_SET_BY_NAME | SDDS_PASS_BY_VALUE,
                                "SdeltaSigma",
                                sqrt(S66_sum2 / (includeDispersion == 1 ? n_good_fits_x : n_good_fits_y) - sqr(S66_sum / (includeDispersion == 1 ? n_good_fits_x : n_good_fits_y))),
                                NULL))))
        SDDS_PrintErrors(stderr, SDDS_EXIT_PrintErrors | SDDS_VERBOSE_PrintErrors);
